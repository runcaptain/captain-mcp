...
export const indexOptionFields = {
  ...
  })).max(20).optional().describe("Customer-defined PII categories to mask in addition to the built-in ones (up to 20). Requires mask_pii; every engine except presidio supports it (presidio rejects it with 422)"),
  pii_instructions: z.string().max(1000).optional().describe("Plain-language guidance on what counts as personal data for this job, e.g. 'staff names may stay; patient names and bed numbers must be masked'. Requires mask_pii; every engine except presidio supports it (presidio rejects it with 422)"),
