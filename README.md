...
- `captain_compact_collection`, `captain_get_compaction`, `captain_retry_compaction`: clone a collection and rebuild the clone's vector-store namespace in the background so it drops attribute names its rows no longer use. Recovers a collection whose namespace hit the 1,024 attribute-name cap, where metadata comes back `filterable: false` and filtered queries fail. Poll `captain_get_compaction` until status is `completed`; `captain_retry_compaction` resumes a run that stopped.
...
- `captain_batch_chunk_metadata` (merge or overwrite chunk metadata, up to 100 chunks per request), `captain_batch_chunk_relations` (create or update up to 100 chunk relations per request; a relation is identified by source, target and relation_type)
