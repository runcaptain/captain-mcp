...
**Job webhooks (2):** Endpoints belong to the organization, so these tools take no `environment` argument. Each tool picks its operation with `action`, and each action checks its own arguments. The five events are `job.completed`, `job.completed_with_errors`, `job.failed`, `job.timed_out` and `job.cancelled`; every indexing job sends exactly one when it reaches its final status.
